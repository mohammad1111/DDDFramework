...
{
    private static readonly IReadOnlyDictionary<string, int> _producerMethods = InitializeProducerMethods();
    private static readonly string _taskNamespace = typeof(Task).Namespace;

    private static IReadOnlyDictionary<string, int> InitializeProducerMethods()
    {
    ...
    {
        return _producerMethods.TryGetValue(
            $"{method.ContainingNamespace}.{method.ContainingType.Name}.{method.Name}", out index);
    }
        ...
        out ITypeSymbol typeArgument)
    {
            ...
            && method.Parameters[0].Type.SpecialType == SpecialType.System_Object)
        {
            ...
            if (index == -1 && method.ContainingType.IsGenericType &&
...
#pragma warning disable RS1024
        return contractTypes.SelectMany(i =>
                i.GetMembers().OfType<IPropertySymbol>()
    ...
    {
        return method.ReturnType.Name == nameof(Task) &&
               method.ReturnType.ContainingNamespace.ToString() == _taskNamespace;
    }

    public static IEnumerable<INamedTypeSymbol> GetAllInterfaces(this ITypeSymbol type)
    {
        var allInterfaces = type.AllInterfaces;
        if (type is INamedTypeSymbol namedType && namedType.TypeKind.IsClassOrInterface() &&
    ...
    {
        var types = GetAllTypes(type);
        var interfaces = GetAllInterfaces(type);

        return types.Any(baseType => SymbolEqualityComparer.Default.Equals(baseType, otherType))
               || interfaces.Any(baseInterfaceType =>
                   SymbolEqualityComparer.Default.Equals(baseInterfaceType, otherType));
    }
}